...
function getDateForCell(row: number, column: number) {
...
export default async function GithubContributionChart({
  ...
  // const contributions = Array.from({ length: 579 }, (_, i) => ({
  //     date: new Date(Date.now() - i * 86400000).toISOString().split("T")[0],
  //     count: Math.floor(Math.random() * 5),
  //     intensity: Math.floor(Math.random() * 5),
  //     color: ["#ebedf0", "#9be9a8", "#40c463", "#30a14e", "#216e39"][
  //         Math.floor(Math.random() * 5)
  //     ] as keyof typeof COLOR_REPLACEMENT_MAP_DARK,
  // }));
          ...
          {Array.from({ length: DAYS_IN_WEEK }, (_, row) => (
            <tr key={row} className='overflow-hidden'>
              ...
              {Array.from({ length: WEEKS }, (_, column) => {
                ...
                // console.log({ contributionIndex, contribution });

                const cellDate = getDateForCell(row, column);

                const contribution = contributionMap.get(cellDate);

                const color = contribution?.color ?? "#ebedf0";

                const darkColor = contribution
                    ? COLOR_REPLACEMENT_MAP_DARK[contribution.color]
